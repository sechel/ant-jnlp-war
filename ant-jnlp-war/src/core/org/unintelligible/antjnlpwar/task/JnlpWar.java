...
public class JnlpWar extends BaseJnlpWar {
    ...
    public void execute() {
        ...
        try {
            ...
            File extensionFolder = createFolder(applicationFolder, "extension");
            ...
            //extensions -> application/extension folder
            for(Iterator it=getExtensions().iterator(); it.hasNext();) {
                Extension ext = (Extension)it.next();
                ExtensionLib extensionLib = new ExtensionLib(this,ext);
                FileScanner scanner=ext.getDirectoryScanner(getProject());
                String[] includedJars=scanner.getIncludedFiles();
                File basedir=scanner.getBasedir();
                for(int i=0; i<includedJars.length;i++){
                    File jar=new File(basedir, includedJars[i]);
                    log("copying extension "+jar);
                    StreamUtil.copyFile(jar, extensionFolder);
                    extensionLib.addJar(jar.getName());
                }
                expandedExtensions.add(extensionLib);
            }
            ...
            if(getSignAlias()!=null){
                if(isPack200()){
                    ...
                    packJars(extensionFolder);
                    unpackJars(extensionFolder);
                }
            ...
            for(Iterator it=expandedExtensions.iterator();it.hasNext();) {
                ExtensionLib lib = (ExtensionLib)it.next();
                File jnlpExtFile = new File(applicationFolder, lib.getJnlpName());
                Generator jnlpExtGenerator = new Generator(lib, jnlpExtFile,
                        "org/unintelligible/antjnlpwar/template/jnlp_extension.vm");
                try {
                    jnlpExtGenerator.generate();
                } catch (Exception e) {
                    log(e.toString());
                    throw new BuildException(
                            "Could not generate the JNLP Extension deployment descriptor for "+lib.getJnlpName(), e);
                }
            }
    ...
    private void signJars(File folder){
        ...
        for(int i=0;i<jarFiles.length; i++){
            ...
            signJarTask.setKeystore(getSignKeystore());
            signJarTask.execute();
